...
class WolfeLS(gradient.GradientDescentMethod):
    ...
    def __wolfe_ls(self, x, gradient):
        while True:
            ...
            obj_alpha = self.evaluate_objective(x - alpha * gradient)
            grad_alpha = np.linalg.norm(self._problem.grad(x - alpha * gradient)) ** 2
            
            # Condizioni di Wolfe forti
            wolfe_1 = obj_alpha <= self.evaluate_objective(x) - self._gamma * alpha * np.dot(gradient, gradient)
            wolfe_2 = grad_alpha <= self._sigma * np.dot(gradient, gradient)
            
            if wolfe_1 or wolfe_2:
                return alpha
            
            if obj_alpha > self.evaluate_objective(x) - self._gamma * alpha * np.dot(gradient, gradient):
                self._alpha_u = alpha
            elif obj_alpha <= self.evaluate_objective(x) - self._gamma * alpha * np.dot(gradient, gradient) and grad_alpha < self._sigma * np.dot(gradient, gradient):
                self._alpha_l = alpha
            elif obj_alpha <= self.evaluate_objective(x) - self._gamma * alpha * np.dot(gradient, gradient) and grad_alpha > self._sigma * np.dot(gradient, gradient):
                self._alpha_u = alpha
